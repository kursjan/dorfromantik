...
- [x] Update `GameBoard.tsx` to manage `saveStatus` state, pass callbacks to `GameAutosaver`, and display a small `SaveStatusIndicator` UI component using **task-conductor** skill.
- [x] Update `GameBoard.test.tsx` to assert the new UI states and verify the component flushes the autosaver on unmount using **task-conductor** skill.

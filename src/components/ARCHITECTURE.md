...
SVG board stack (**`SvgBoardCameraShell`** + **`SvgBoard`**) lives next to **`SvgGameView`** under **`src/rendering/svg/components/`**; per-tile primitives (`HexTile`, wedges, segment renderers, `SvgHexUtils`) under **`src/rendering/svg/tiles/`**. Canvas parity uses **`src/rendering/canvas/components/TilePreview.tsx`**. Layout uses `SvgHexUtils.hexToPixel`, delegating to **`src/rendering/common/hex/HexUtils.hexToPixel`** with `SVG_HEX_RADIUS`. Camera, **`CameraSnapshot`**, pointer spaces (**`ContainerPoint`** / **`ContainerDelta`** / **`WorldPoint`**), and SVG vs canvas wiring are documented in **`src/rendering/ARCHITECTURE.md`**.

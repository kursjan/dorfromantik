...
- [ ] Create a Pull Request on GitHub and assign it to the user for code review. - You MUST NOT merge the PR yourself. The user is responsible for the final review and merge on GitHub.
...
LGTM (SGTM) means: the current task sent for approval is approved. That usually corresponds to the task you just marked `[x]` after the work was verified; the agent should have ended with **`STATUS: WAITING_FOR_APPROVAL`** (task-conductor) or **`STATUS: PHASE_COMPLETED_WAITING_FOR_REVIEW`** (project-orchestrator phase gate), and LGTM approves the work tied to that status.

Continue means: pick the next task and execute and send for approval.

Any other comment means: the current task is still considered as "in progress" by the user and changes are required, until LGTM.

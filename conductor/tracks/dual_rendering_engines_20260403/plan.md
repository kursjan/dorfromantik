...
- [ ] Add an **`requestAnimationFrame` loop** (or equivalent) on the SVG path that applies **continuous camera rotation** from held Q/E, mirroring `CanvasController.processContinuousInput`, using the same `Camera` math and `useGameSnapshotBridge` / game mutation wiring as `CanvasView`. using **task-conductor** skill.
...
## Phase 3: Settings & Dual Rendering

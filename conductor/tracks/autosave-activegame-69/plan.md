...
- [ ] Create and switch to feature branch `worker1` using **task-conductor** skill.
...
- [ ] Remove autosave coupling to canvas `onTilePlaced` callback path (`CanvasView`/`GameBoard`) using **task-conductor** skill.
- [ ] Implement `GameBoard` transition-based trigger using existing `activeGameRef` and a domain helper (e.g. `didGameplayStateChange`) using **task-conductor** skill.
...
- [ ] Add/update `GameBoard` autosave transition tests (meaningful transition triggers save, non-meaningful transition does not) using **task-conductor** skill.
- [ ] Add/update regression tests for rapid transitions/debounce and unmount flush using **task-conductor** skill.
